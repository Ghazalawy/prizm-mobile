...
export function ReportCreateScreen({ preselectedProjectId }: Props) {
  const router = useRouter();
  const createMut = useCreateReport();
  const uploadMut = useUploadReportImages();
  const { data: projects, isLoading: projectsLoading } = useReportProjects();

  const [step, setStep] = useState(0);

  // Step 1 — Header
  const [projectId, setProjectId] = useState<number | undefined>(preselectedProjectId);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (newId && photos.length > 0) {
        const uploadItems: ImageUploadItem[] = photos.map((p) => ({
          uri: p.uri,
          description: p.description,
        }));
        await uploadMut.mutateAsync({ reportId: newId, images: uploadItems });
      }

      Toast.show({
        type: "success",
        text1: "Report created",
        text2: newId ? `ID: ${newId}` : undefined,
      });

      if (newId) {
        router.replace(`/(tabs)/reports/${newId}` as any);
      } else {
        router.back();
      }
    } catch (err: any) {
    ...
    projectId, reportDate, scopeDescription, outstandingIssues, suggestions,
    workItems, nextItems, photos, createMut, uploadMut, router,
  ]);
        ...
        >
          ...
          {step === 1 && (
            <StepWorkItems
              ...
              subtitle="Record activities completed today"
              items={workItems}
              percentLabel="Today %"
          ...
          {step === 2 && (
            <StepWorkItems
              ...
              subtitle="Plan upcoming work activities"
              items={nextItems}
          ...
          <View key={idx} className="flex-row items-center flex-1">
            ...
            {idx < total - 1 ? (
              <View
                ...
                style={{
                  backgroundColor: idx < current ? colors.success : colors.slate200,
                }}
              />
            ) : null}
          </View>
  ...
  return (
    <View>
      ...
      <Text className="text-sm text-slate-500 mb-4">
        Select the project and date for this report.
      </Text>
      ...
      <TextInput
        ...
        placeholder="Describe the scope of work for this report…"
        placeholderTextColor={colors.slate400}
        multiline
...
function StepIssues({
  ...
  return (
    <View>
      ...
      <Text className="text-sm text-slate-500 mb-4">
        Document any blockers or improvement ideas.
      </Text>
      ...
      <TextInput
        ...
        placeholder="List any outstanding issues, delays, or blockers…"
        placeholderTextColor={colors.slate400}
        multiline
        textAlignVertical="top"
...
function StepPhotos({
  ...
  return (
    <View>
      ...
      <Text className="text-sm text-slate-500 mb-4">
        Capture or select photos to document work progress.
      </Text>
      ...
      {photos.length === 0 ? (
        <View className="items-center py-10">
          ...
          <Text className="text-slate-500 text-sm text-center">
            No photos yet. Tap Camera to capture site photos.
          </Text>
        </View>
      ) : (
...
function StepReview({
  ...
  return (
    <View>
      <Text className="text-lg font-bold text-slate-900 mb-1">Review & Submit</Text>
      <Text className="text-sm text-slate-500 mb-4">
      ...
      <ReviewSection title="Report Details" icon="document-text-outline">
        ...
        <ReviewRow label="Client" value={selectedProject?.client_name ?? "—"} />
...
function ReviewRow({
  ...
  return (
    <View className={`flex-row ${multiline ? "flex-col" : "items-center justify-between"} mb-1.5`}>
